...
class Transcript():
	...
	def write_transcript(self, end=0):
		...
		writer = csv.writer(open(self.outputFileName,encoding='utf-8', mode= 'w'))
		writer.writerow(["SentenceNo","SequenceNo","Date","Time","Speaker","Text"])
		for i in range(len(self.messagelist[:end])):
			print('processed',[i,self.paragraphList[i],self.datelist[i], self.timelist[i],self.speakerlist[i], self.messagelist[i]])
